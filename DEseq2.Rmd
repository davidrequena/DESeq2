...
cols[(res$log2FoldChange >= 1) & (res$padj < 0.05)] <- "red"
cols[(res$log2FoldChange <= -1) & (res$padj < 0.05)] <- "blue"
cols[(abs(res$log2FoldChange) >= 1) & (res$padj >= 0.05)] <- "grey60"

# Condition to select just a group of genes
cond1 <- !( (abs(res$log2FoldChange) <= 1) | (res$padj > 0.05) )
cond2 <- !( (abs(res$log2FoldChange) <= 3) & (res$padj > 0.0001) )
cond <- cond1 & cond2
cond[is.na(cond)] <- FALSE

# To fill the empty labels
res$symbol[is.na(res$symbol) | (res$symbol == "")] <- "NS"
...
# abline(h = -log10(0.0001), col = "grey75")
# abline(v = c(-3, 3), col = "grey75")
# 
# # To add a title
...
#############################################
p.volcano <- ggplot() + theme_bw() + scale_x_continuous(expand=c(0,0)) +
  scale_y_continuous(expand=c(0,0), limits = c(0, max(-log10(res$padj), na.rm = TRUE) + 1) ) +
  geom_vline(xintercept = c(-1,1), colour = "darkgreen", alpha = 0.7, linetype = 2) +
  geom_hline(yintercept = c(-log10(0.05)), colour = "darkgreen", alpha = 0.7, linetype = 2) +
  geom_point(data = data.frame(res),
             aes(x = res$log2FoldChange, y = -log10(res$padj)), color = cols,
             size = 1, na.rm = TRUE, show.legend = TRUE) +
  geom_text_repel(aes( x = res$log2FoldChange[cond], y = -log10(res$padj[cond]), label = res$symbol[cond] ),
                  ...
                  box.padding = unit(0.1, "lines"), min.segment.length = unit(0.2, "lines")) +
  labs(x = expression("log"[2]*"(Fold Change)"),
       y = expression("-log"[10]*"(p adjusted)"),
       title = "Metastasis / Primary tumors") +
  scale_colour_manual(name = "Legend", 
                      breaks = c("Overexpressed", "Underexpressed", "Not significant", "Fold change < 2"),
                      values = c("red", "blue", "grey60", "black"))
