---
title: "Guide for Differential Expression Analysis using DEseq2"
...
# Guide for Differential expression analysis using DEseq2
...
This block is to explore the distribution of normalized counts by sample
```{r, echo=FALSE}
#############################################################
# Exploring the distribution of normalized counts by sample #
#############################################################

# List of samples  by type
type_all <- sampledata$id
type_normal <- sampledata$id[sampledata$sample_type == "normal"]
type_primary <- sampledata$id[sampledata$sample_type == "primary"]
type_metastasis <- sampledata$id[sampledata$sample_type == "metastasis"]

# Normalize the dataset:
library_sizes <- colSums(countstable.num)
countstable.normalized <- countstable.num / colSums(countstable.num)

# To check the counts by sample type:
countstable.N.means <- rowMeans(countstable.normalized[ , type_normal])
countstable.P.means <- rowMeans(countstable.normalized[ , type_primary])
countstable.M.means <- rowMeans(countstable.normalized[ , type_metastasis])

df.mean_counts <- data.frame(Normal = countstable.N.means,
                             Primary = countstable.P.means,
                             Metastasis = countstable.M.means)

genemap <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol", "description"),
                 filters = "ensembl_gene_id",
                 values = rownames(df.mean_counts),
                 mart = useMart("ensembl", dataset = "hsapiens_gene_ensembl"))

indx <- match(rownames(df.mean_counts), genemap$ensembl_gene_id)
df.mean_counts$symbol <- genemap$hgnc_symbol[indx]
df.mean_counts$description <- genemap$description[indx]

write.csv(as.data.frame(df.mean_counts), file = "Mean_Counts.csv")
```
...
Prior Filtering
```{r, echo=FALSE}
# You might want to remove also those genes with counts in only one sample.
# But there should be a rationale for this. Check first those cases,
...
p.tsne_features <- ggplot(data = df.tsne) + theme_bw() +
  ...
  scale_fill_manual(values = nice_colors, guide = guide_legend(override.aes = aes(shape = 21))) +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(), legend.position = c(0.95, 0.84),
...
write.csv(as.data.frame(res_NM), file = "results_Mets_vs_Normal_full.csv")
write.csv(as.data.frame(res_NP), file = "results_Prims_vs_Normal_full.csv")
...
write.csv(as.data.frame(res_NP.sig.fold_over),
...
write.csv(as.data.frame(res_NP.sig.fold_under),
...
write.csv(as.data.frame(res_NM.sig.fold_over),
...
write.csv(as.data.frame(res_NM.sig.fold_under),
...
# Optional: To remove some extrene values, to reduce the range of the volcano plot for posters
...
plotBoxScatter_DR <- function (genename = NULL, symbol = NULL, colors = NULL, poster = NULL)
{
  ...
  if (poster == TRUE) {
    ...
    p.bs <- p.bs +
      geom_point(aes(fill = df.box$sample_type, shape = df.box$lib_prep),
                 size = 7, alpha = 0.8, color = "black",
                 position = position_jitter(width = 0.35)) +
      theme(axis.text.x = element_text(size = 20),
            axis.text.y = element_text(size = 16)) +
      scale_shape_manual(name = "Library", values = c(24, 21, 22, 25)) +
...
Code in progress:
```{r, echo=FALSE}
data <- plotCounts(ds.deseq, gene = "ENSG00000134827", intgroup=c("lib_prep", "sample_type", "patient"), returnData=TRUE)

ggplot(data, aes(x = sample_type, y = count, color = lib_prep)) +
  scale_y_log10() + 
  geom_point(position=position_jitter(width=.1,height=0))

ggplot(data, aes(x = sample_type, y = count, color = lib_prep, group = patient)) +
  scale_y_log10() + geom_line() + geom_point(position=position_jitter(width=.1,height=0))
```
