...
plotPCAdr <- function (object, intgroup = "condition", ntop = 500, PCs = c(1,2), size = 7, alpha = 1,
                       colors = NULL, returnData = FALSE) 
{
...
# nice_colors <- c("dodgerblue3", "gold", "firebrick3")
nice_colors <- c("steelblue1", "khaki1", "firebrick1")
...
# PC 1 vs 2
pca12 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), PCs = c(1,2),
...
# PC 2 vs 3
pca23 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), PCs = c(2,3),
                   colors = nice_colors, ntop =  nrow(assay(transf.data))) +
  labs(fill = expression(underline("Sample Type")), shape = expression(underline("Library"))) +
  geom_text_repel(aes_string(label = transf.data$id), color = "black", cex=4,
                             min.segment.length = unit(0.1, "lines"), box.padding = unit(0.6, "lines"))
# PC 3 vs 4
pca34 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), PCs = c(3, 4),
                   colors = nice_colors, ntop =  nrow(assay(transf.data))) +
  labs(fill = expression(underline("Sample Type")), shape = expression(underline("Library"))) +
  geom_text_repel(aes_string(label = transf.data$id), color = "black", cex=4,
                             min.segment.length = unit(0.1, "lines"), box.padding = unit(0.6, "lines"))
...
# To save the figures:
ggsave("Plots/PCA12_clean.jpg", plot(pr$x[,], pch = 16), width = 16, height = 10, dpi = 300)
ggsave("Plots/PCA12_features.jpg", pca12, width = 16, height = 10, dpi = 300)
ggsave("Plots/PCA23_clean.jpg", plot(pr$x[,-1], pch = 16), width = 16, height = 10, dpi = 300)
ggsave("Plots/PCA23_features.jpg", pca23, width = 16, height = 10, dpi = 300)
ggsave("Plots/PCA34_clean.jpg", plot(pr$x[,-c(1,2)], pch = 16), width = 16, height = 10, dpi = 300)
...
p.tsne_big <- ggplot(data = df.tsne, aes(x = X1, y = X2, shape = library, fill = sample_type)) +
...
ggsave("Plots/tSNE_big.jpg", p.tsne_big, width = 8.5, height = 7, dpi = 300)
```
