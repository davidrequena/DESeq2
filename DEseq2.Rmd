...
black_list <- data.frame(read.csv("black_list.tsv", sep = "\t", header = TRUE))
countstable <- countstable[!(countstable[,1] %in% black_list[,1]), ]
...
# Make PCA plots:
# PC 1 vs 2
pca12 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), ntop =  nrow(assay(transf.data)), PCs = c(1,2))
...
# PC 2 vs 3
pca23 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), ntop =  nrow(assay(transf.data)), PCs = c(2,3))
pca23 <- pca23 + geom_text_repel(aes_string(label = transf.data$id), color = "black",
                             cex=3, nudge_x = -4, nudge_y = 0,
                             min.segment.length = unit(0.1, "lines"), box.padding = unit(0.4, "lines"))
# PC 3 vs 4
pca34 <- plotPCAdr(transf.data, intgroup = c("sample_type", "lib_prep"), ntop =  nrow(assay(transf.data)), PCs = c(3,4))
...
p.volcano <- ggplot() + theme_bw() +
  ...
  geom_text_repel(aes( x = d.volcano$log2FoldChange[cond], y = -log10(d.volcano$padj[cond]), label = d.volcano$symbol[cond] ),
...
plotDR_bygene <- function (genename = NULL, symbol = NULL)
{
  ...
  df.box$sample_type <- factor(df.box$sample_type, c("normal", "primary", "metastasis"))
  ...
  p.bs <- ggplot(df.box, aes(x = df.box$sample_type, y = log2(df.box$gene_counts))) +
    theme_bw() + geom_boxplot(width = 0.4, fill = "gray90") +
    geom_point(aes(fill = df.box$sample_type),
               size = 3, alpha = 0.75, pch = 21, color = "black",
