...
# create a DESeqDataSet from the tables above, with no specific design yet:
...
#This step is to indicate the base level: sample_type == normal
str(colData(ds.deseq)$sample_type) # To see which is the current base level
colData(ds.deseq)$sample_type  <- relevel(colData(ds.deseq)$sample_type, "normal")
str(colData(ds.deseq)$sample_type) # To verify which is the current base level
```
...
# plotPCAdr function #
######################
# This is the plotPCA function with some improvements made by David Requena
# To chosse which PCs plot and to use two features for color/shape
...
# Define the model (design) to use, controlling by:
...
### Comparison
```{r, echo=FALSE}
# Run DESeq2 using the model defined
...
# Using a different HeatMap function:
vsd <- getVarianceStabilizedData(ds.deseq)
vsdp <- vsd[rownames(vsd) %in% rownames(res_sig),]
pheatmap(vsd, cluster_rows = TRUE, show_rownames = FALSE, #kmeans_k = 1000,
         annotation = data.frame(sampledata[,c(1,4)]))
pheatmap(vsdp, cluster_rows = TRUE, show_rownames = FALSE, #kmeans_k = 100,
         annotation = data.frame(sampledata[,c(1,4)]))
```
